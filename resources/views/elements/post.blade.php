                                    ...
                                    if($share->post_id == $post->id){
                                        ...
                                        <div class="w-full flex-none mb-2 text-xs text-blue-700 font-medium" wire:offline.class.remove="text-blue-700" wire:offline.class="text-gray-400">
                                            <a href="{{ route('profile', ['username' => $name]) }}">
                                                <?php
                                                    //Profile Picture
                                                    $id = App\Models\User::where('username', $name)->pluck('id');
                                                        
                                                    $photo = App\Models\User::whereIn('id', $id)->get(); 
                                                        
                                                    foreach($photo as $sharedbyPic){
                                                        ?>	
                                                            <img src="{{ $sharedbyPic->profile_photo_url }}" alt="{{ $sharedbyPic->username }}" class="inline-block object-cover w-8 h-8 mr-1 text-white rounded-full shadow-sm cursor-pointer" 
